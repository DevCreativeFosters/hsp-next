...
export function formatPrice(price, sign = '$') {
  // Always emit two decimal places. Without this, whole-dollar
  // amounts come back as "$3,150" while tier-priced amounts come
  // back as "$2,362.5", and downstream callers used to paper over
  // it by literally appending ".00" — which then rendered as
  // "$2,362.5.00" the moment a non-integer price showed up.
...
export function makeRelativeUrl(url) {
  if (!url) return url;
  return url.replace(
    /^https?:\/\/[^/]*(hsputelids\.com|cloudwaysapps\.com)[^/]*/,
    '',
  );
}

export function accountSlug(role) {
  // Three distinct frontend roles, three distinct account routes:
  //   retail → /account/retail
  //   b2b    → /account/b2b
  //   dealer → /account/dealer
  // Previously this collapsed b2b into 'dealer', which sent b2b accounts
  // to /account/dealer that doesn't exist on every environment.
