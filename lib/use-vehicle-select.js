...
export const useVehicleSelection = (
  ...
  const handleModelChange = useCallback(
    ...
    [setCheckingProductCompatibility, setModel],
  );
  ...
  useEffect(() => {
    const savedSelection = sessionStorage.getItem(LOCAL_STORAGE_VEHICLE);
    if (savedSelection) {
      ...
      const contextMakeSlug = getValueOrSlug(savedVehicle?.maker);
      const contextModelSlug = getValueOrSlug(savedVehicle?.model);

      setCheckingProductCompatibility(true);

      if (contextMakeSlug) {
        checkProductCompatibility(slug, contextMakeSlug, contextModelSlug);
      }
    }
