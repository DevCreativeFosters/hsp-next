...
export async function render(block) {
  ...
  if ('PostsCarousel' === blockName) {
    ...
    if (block.postType !== 'hsp-tv') {
      ...
      postsList = posts?.hspTvPosts?.nodes;
    }
    ...
    const buttons = [
      {
        label: block.viewAllButtons?.title,
        variant: 'primary',
        link: block.viewAllButtons?.url,
      },
    ];
