...
// Same name/email/phone source the /account "My Details" page and the
// /checkout autofill use. Retail onboarding users have no WooCommerce
// billing yet, so this is the only place their contact details live.
const VENDOR_PROFILE_QUERY = `
  query($userId: Int) {
...
export async function prefetchCheckoutContact(user) {
  ...
  try {
    ...
    if (user.role === 'b2b') {
      ...
      // Onboarding-only retail users have no WC billing, so user.email
      // is undefined and userAddress is empty. Pull vendorProfile(userId)
      // for name/email/phone so the primed cache isn't blank on the
      // first /checkout paint. Tokenless, matching contexts/user.js.
      const vpRes = await fetchAPI(VENDOR_PROFILE_QUERY, {
        variables: { userId: Number(user.id) },
      });
      const vProfile = vpRes?.vendorProfile || null;

      const lookupEmail = user.email || vProfile?.email || '';
      ...
      if (src?.address_1 || src?.city || src?.postcode || profile || vProfile) {
        shipping = {
          ...
          phone: src?.phone || profile?.phone || vProfile?.phone || '',
