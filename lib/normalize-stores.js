...
export default function normalizeStores(stores, searchGeolocation) {
  ...
  return sortedStores.map(record => {
    ...
    return {
      address: street,
      assignedB2bUser: record.assignedB2bUser,
      availableDeliveryOptions:
        record?.storesCustomFields?.availableDeliveryOptions,
      billingAddress: record?.storesCustomFields?.billingAddress,
      blocks: record.storesCustomFields.blocks,
      color,
      deliveryAddress: record?.storesCustomFields?.deliveryAddress,
      ...
      marketingResources: record.storesCustomFields.marketingResources,
      name: title,
