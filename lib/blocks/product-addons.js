...
export default function ProductAddonsBlock(block) {
  ...
  const { description, productAddons, title, titleTag, titleTagStyle } = block;
    ...
    switch (type) {
      ...
      case 'productMake':
        const makeDetails = entity.nodes
          .flatMap(node =>
            node.detailsFields?.details?.map(detail => {
              const matchingCategoryId =
                detail?.relatedProductCategory?.nodes?.[0]?.id ?? null;
              return {
                ...node,
                detail,
                matchingCategoryId,
          ...
          .find(
            item =>
              item?.matchingCategoryId &&
              relatedCategoryIds.includes(item.matchingCategoryId),
          );
