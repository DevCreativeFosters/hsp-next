import { AccordionFacts } from '@lib/api-acf-blocks/accordion-facts';
import { Accreditations } from '@lib/api-acf-blocks/accreditations';
// testing-only: ButtonLinkTag + CarouselFlexibleContentBlock layouts don't
// exist on testing WP's Flexible Content schema, so including their fragments
// makes WPGraphQL reject every page query with "Unknown type ..." and the
// Vercel prerender step fails on / , /products, /lifestyle, /account/*, etc.
// Strip both from the registry here until the testing backend syncs. REVERT
// before merging to main/staging where the types DO exist.
// import { ButtonLinkTag } from '@lib/api-acf-blocks/button-link-tag';
// import { CarouselFlexibleContentBlock } from '@lib/api-acf-blocks/carousel-flexible-content-block';
import { CategoriesAndProducts } from '@lib/api-acf-blocks/categories-and-products';
import { ChooseYourVehicle } from '@lib/api-acf-blocks/choose-your-vehicle';
import { ClickableLogos } from '@lib/api-acf-blocks/clickable-logos';
import { ColumnsFacts } from '@lib/api-acf-blocks/columns-facts';
import { Faq } from '@lib/api-acf-blocks/faq';
import { FeaturePanels } from '@lib/api-acf-blocks/feature-panels';
import { FeaturesValues } from '@lib/api-acf-blocks/features-values';
import { Hero } from '@lib/api-acf-blocks/hero';
import { ImageBlock } from '@lib/api-acf-blocks/image-block';
import { ImageTextBoxes } from '@lib/api-acf-blocks/image-text-boxes';
import { InformationCards } from '@lib/api-acf-blocks/information-cards';
import { InstagramFeed } from '@lib/api-acf-blocks/instagram-feed';
import { IntroAndCards } from '@lib/api-acf-blocks/intro-and-cards';
import { Lifestyle } from '@lib/api-acf-blocks/lifestyle';
import { LinksInGroups } from '@lib/api-acf-blocks/links-in-groups';
import { PostsCarousel } from '@lib/api-acf-blocks/posts-carousel';
import { ProductAddons } from '@lib/api-acf-blocks/product-addons';
import { ProductGrid } from '@lib/api-acf-blocks/product-grid';
import { ProductMakeGrid } from '@lib/api-acf-blocks/product-make-grid';
import { ProductTiles } from '@lib/api-acf-blocks/product-tiles';
import { PromoImageAndText } from '@lib/api-acf-blocks/promo-image-and-text';
import { PromoTextAndVideo } from '@lib/api-acf-blocks/promo-text-and-video';
import { PromoWith2Videos } from '@lib/api-acf-blocks/promo-with-2-videos';
import { Reviews } from '@lib/api-acf-blocks/reviews';
import { Shortcode } from '@lib/api-acf-blocks/shortcode';
import { SidebarLinkGroup } from '@lib/api-acf-blocks/sidebar-link-group';
import { Tiles } from '@lib/api-acf-blocks/tiles';
import { TitleAndDescription } from '@lib/api-acf-blocks/title-and-description';
import { VideoBackgroundHero } from '@lib/api-acf-blocks/video-background-hero';
import { VideoCarousel } from '@lib/api-acf-blocks/video-carousel';
import { VideoEmbed } from '@lib/api-acf-blocks/video-embed';

export const blocks = {
  AccordionFacts,
  Accreditations,
  // testing-only strip (see imports above):
  // ButtonLinkTag,
  // CarouselFlexibleContentBlock,
  CategoriesAndProducts,
  ChooseYourVehicle,
  ClickableLogos,
  ColumnsFacts,
  Faq,
  FeaturePanels,
  FeaturesValues,
  Hero,
  ImageBlock,
  ImageTextBoxes,
  InformationCards,
  InstagramFeed,
  IntroAndCards,
  Lifestyle,
  LinksInGroups,
  PostsCarousel,
  ProductAddons,
  ProductGrid,
  ProductMakeGrid,
  ProductTiles,
  PromoImageAndText,
  PromoTextAndVideo,
  PromoWith2Videos,
  Reviews,
  Shortcode,
  SidebarLinkGroup,
  Tiles,
  TitleAndDescription,
  VideoBackgroundHero,
  VideoCarousel,
  VideoEmbed,
};
