...
export default function ProductMakeGridBlock(block, options = {}) {
  if (!block) return null;

  const { isCategoryPage = false } = options;
  ...
  const showFilters = isCategoryPage ? true : block?.showFilters;
